...
class Controller_player:
    ...
    def choose_players(self, tournoi):
        ...
        for row in db_players:
            player = pl.Player(row['player_id'], row['lastname'], row['firstname'], row['birthdate'],
    ...
    def update_score_if_replay(self, tournoi):
        ...
        for indr in range(len(tournoi.rounds)):
            for indg in range(len(tournoi.rounds[indr].games)):
                inda = get_result_player(tournoi.players, tournoi.rounds[indr].games[indg].player_a)
                indb = get_result_player(tournoi.players, tournoi.rounds[indr].games[indg].player_b)
