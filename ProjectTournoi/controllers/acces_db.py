...
class controller_db:
        ...
        for row in db_tournament:
            ...
            for indp in range(len(row['players'])):
                player_sel = pl.Player(row['players'][indp]['player_id'],
                                       row['players'][indp]['lastname'],
                                       row['players'][indp]['firstname'],
                                       row['players'][indp]['birthdate'],
                                       row['players'][indp]['sex'],
                                       row['players'][indp]['classment'],
                                       row['players'][indp]['score'])
                tournament.players.append(player_sel)
            """create rounds"""
            for indr in range(len(row['rounds'])):
                round_sel = rn.Round(row['rounds'][indr]['round_id'],
                                     row['rounds'][indr]['begindate'],
                                     row['rounds'][indr]['begintime'],
                                     row['rounds'][indr]['enddate'],
    ...
    def update_player(self, player):
        Playerid = Query()
        db_players.update({'classment': player.classment,
