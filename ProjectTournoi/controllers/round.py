...
class turning_round:
    def turning_round(self, num_round, tournament):
        ...
        """Swiss algorithm"""
        if cgm.Controller_game.get_games_swiss(self,
                                               num_round, tournament) is False:
            return False
        running_game = True
        while running_game:
            """view games"""
            rep.Controller_reports.print_turning_views(self,
                                                       num_round, tournament)
            """choose number of game to encode or leave the round"""
            running_game, igame = cgm.Controller_game.\
                get_game_choose(self, num_round)
            """encode score"""
            if running_game:
                cgm.Controller_game.get_result(self,
                                               tournament.rounds[num_round].
                                               games[igame])
        """end round"""
